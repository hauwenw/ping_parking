...
- **AC4**: On creation, system atomically:
  - Sets status = "pending" if start_date > current_date, "active" if start_date <= current_date
  - Auto-generates payment record (amount, due_date=start_date, status="pending")
  - Space status remains computed (occupied only if has active agreement RIGHT NOW)
  - Logs to system_logs
...
- **Initial Status**:
  - `start_date > current_date` → status="pending"
  - `start_date <= current_date` → status="active"
- **Payment Generation**: Created at agreement creation (not activation) with due_date=start_date, status="pending"
...
**Fields**: Customer (searchable dropdown), Space (available only), License Plates (required, "+ 新增車牌" button for additional plates), Type (radio: 日/月/季/年), Start Date (date picker), Price (read-only, auto-calculated)

## Source
init_draft.md lines 26, 67, 104-107 | CLAUDE.md agreement lifecycle

## Dependencies
US-CUST-001, US-SPACE-001, US-PAY-002, US-AUDIT-002, US-LOC-003, US-LOC-004
